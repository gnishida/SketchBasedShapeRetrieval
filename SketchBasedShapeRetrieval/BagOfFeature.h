...
public:
	BagOfFeature(const cv::Mat& image, const std::string& filepath, const Camera& camera, float sigma, float lmbd);

	void computeHistogram(const std::vector<cv::Mat>& visualWords);
	void extractFeatures(const cv::Mat& image, float sigma, float lmbd);
